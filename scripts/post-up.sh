...
blobs_extension=$(az functionapp keys list -n ${AZURE_PROCESSOR_FUNCTION_APP_NAME} -g ${AZURE_RESOURCE_GROUP} --query "systemKeys.blobs_extension" -o tsv)
...
az eventgrid system-topic event-subscription create -n "audio-files-topic-subscription" -g "${AZURE_RESOURCE_GROUP}" --system-topic-name "${AUDIOS_EVENTGRID_SYSTEM_TOPIC_NAME}" --endpoint-type "webhook" --endpoint "$endpointUrl" --included-event-types "Microsoft.Storage.BlobCreated" --subject-begins-with "$filter" 
